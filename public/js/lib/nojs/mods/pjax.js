...
define("lib/nojs/mods/pjax", [ "lib/jquery/jquery" ], function(require) {
    ...
    //检测浏览器是否支持pjax
    pjax.support = window.history && window.history.pushState && window.history.replaceState && // pushState isn't reliable on iOS until 5.
    ...
    pjax.prototype = {
        ...
        pjax: function(event) {
            var top = this.container.offset().top, target = this.target;
            if (typeof this.options.scrollTop == "number") {
                top = this.options.scrollTop;
            }
            if (target && target.hash) {
                var _target = $(target.hash);
                if (_target.length) {
                    $(window).scrollTop(_target.offset().top);
                }
            }
            $(window).scrollTop(top);
            typeof this.options.start == "function" && this.options.start.call(this);
            var url = target.href, _options = this.options;
            ...
            var self = this, options = $.extend(true, {}, $.ajaxSettings, _options.ajaxSettings), timer = _options.timeout, xhr, timeoutTimer;
            if (pjax.support) {
                pjax.state = {
                    index: this.index,
