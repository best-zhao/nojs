 ...
 * 2013-6-4
 */
define("lib/nojs/mods/upload", [ "lib/jquery/jquery" ], function(require) {
    var $ = require("lib/jquery/jquery"), agent = navigator.userAgent.toLowerCase(), browser = {
        "0": window.FileReader,
    ...
    function upload(button, options) {
        ...
        this.options = opt = getOptions(options);
    ...
    upload.prototype = {
        ...
        Events: function(event, data) {
            var T = this, id, e = T.options[event], args = Array.prototype.slice.call(arguments, 1, arguments.length);
            ...
            if (event == "onSelect" && e) {
                ...
                T.fileItem[id].wrap.find(".cancel").click(function(e) {
                    T.cancelUpload(this.id || $(this).data("id"));
                    return false;
                });
            }
            return e;
        ...
        uploading: function(id) {
            ...
            if (browser["4"]) {
                ...
                var name = "uploadiframe" + +new Date(), callback = opt.jsoncallback || {
                    key: "jsoncallback",
                    value: "upload" + +new Date()
                }, action = opt.uploader + (opt.uploader.indexOf("?") < 0 ? "?" : "&"), //formData = '<input type="hidden" name="uploadID" value="'+id+'" />',
                formData = "", host = opt.host || location.host;
