...
define("lib/nojs/mods/chart", [ "lib/jquery/jquery" ], function(require) {
    ...
    chart.prototype = {
        ...
        count: function() {
            var width = this.canvas.width(), height = this.canvas.height(), offset = this.offset, data = this.data.items, n = data.length, i = 0, max = [], min = [], _data;
            for (;i < n; i++) {
                _data = data[i].data;
            ...
            var per = (max - min) / (height - offset.top - offset.bottom);
            //每像素数值
            ...
            max = max - (maxY == 0 ? 0 : maxY - this.dataStep);
            //纵向栅格最大值
        ...
        coordinate: function() {
            //生成坐标系
            var width = this.attr.width, height = this.attr.height, max = this.attr.max, min = this.attr.min, offset = this.offset, i;
            //横向坐标轴X
            ...
            //栅格X
            var gridY = (max - min) / this.dataStep;
            //纵向栅格数
        ...
        draw: function(percent) {
            this.coordinate();
            //数据展示
            var data = this.data.items, n = data.length, width = this.canvas.width() - this.offset.right, height = this.canvas.height() - this.offset.bottom, //
            _height = height - (this.grid.minY ? this.grid.stepY : 0), self = this, arcPoint = [];
            percent = percent || 1;
        ...
        drawLine: function(point, options) {
            //绘制线条
            var i, n = point.length, canvas = canvasSupport ? this.ctx : this.canvas;
            options = options || {};
            if (canvasSupport) {
                canvas.save();
                canvas.beginPath();
                ...
                var path = "", v = "", lineWidth = options.lineWidth || 1, opacity = options.fill && options.fill.match(/(\d+,\d+,\d+),([\.\d]+)/), rgb;
                if (opacity && opacity[2]) {
                    rgb = opacity[1].split(",");
                    options.fill = chart.RgbToHex(rgb[0], rgb[1], rgb[2], true);
                    //rgba转为16进制
                    opacity = "filter:alpha(opacity=" + opacity[2] * 100 + ")";
                } else {
                    opacity = "";
                }
                for (i = 0; i < n; i += 2) {
    ...
    chart.createvml = function() {
        if (!chart.createvml.init) {
            var s = document.createStyleSheet(), shapes = [ "polyline", "oval", "arc", "stroke", "shape" ];
    ...
    function animationLoop(Chart) {
        var amount = 1 / 60, easingFunction = animationOptions[Chart.options.animationEasing], complete = 0;
