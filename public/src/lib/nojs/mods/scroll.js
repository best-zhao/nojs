...
define(function(require){
	...
	function scroll(element, opt){
		...
		this.opt = opt = opt || {};
		this.direction = opt.direction || 'y';//滚动方向，默认纵向
		this.step = typeof opt.step!=='undefined' ? parseInt(opt.step) : 1;//滚动步长，0为连续滚动
		this.time = opt.time || (this.step ? 6000 : 30); //滚动速度，连续推荐设置40ms ;间断滚动时，该值为滚动的间隔时间
		this.size = null;
		this.view = null;	//可见区域的个数	
		
		this.repeat = opt.repeat==false ? false: true;//是否重复循环无间断
		this.auto = opt.auto==false ? false : true;
		
		this.index = 0;//当前位置起始index
	...
	scroll.prototype = {
		init : function(){
			...
			this.totalLength = this.len;//总个数 包含追加
            this.scrollLength = 0;//已滚动个数
            ...
            //mobile touch
            if( screen.width<=640 ){
                var prevAction = T.direction=='y' ? 'swipeDown' : 'swipeRight',
                    nextAction = T.direction=='y' ? 'swipeUp' : 'swipeLeft';
        ...
        append : function(start, length){
            ...
            if( length > last ){
                copy = this.item.slice(start).clone();//从当前copy到结尾
                start = 0;
            ...
            this.totalLength = this.con.children().length;//追加后的总个数
		...
		scroll : function(next){
            ...
            var T = this,
                ...
                //计算最大滚动差
                max = T.size.total - T.size.box,
                scrollAttr = this.direction == 'x' ? 'scrollLeft' : 'scrollTop',
                attr = {},
                now = this.wrap[scrollAttr](),
                nowScroll,
                ratio = next ? 1 : -1;
            ...
            if( this.step ){
                ...
                //this.scrollLength = getIndex(Math.round(now/T.size.item), T.len);
                
            }else{
                //连续滚动
                this.scrollLength = Math.floor(now/T.size.item);
            }
            this.index = this.scrollLength % this.len;//当前开始index
            ...
            this.endIndex = getIndex(this.index+this.view-1, this.len);//当前结束index
            ...
            this.wrap.animate(attr, speed, 'easeOutExpo', function(){
                ...
                if( last < T.view ){//需再次追加 此处step=0不会存在
		...
		reset : function(){//window reset时更新this.size/this.view 
