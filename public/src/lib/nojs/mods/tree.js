...
define(function(require){
	...
	tree.ajax = function( options ){
		...
		var data = options.data,
		    url = options.url,
		    parent = data && data[tree.key.parent];//父id
		    
		if( url.indexOf('?')==-1 && !data ){//url中没有带参数名 则加载根目录数据tree.rootID
	...
	tree.select = function( box, options ){
		...
		addItem.empty = function(level){
		    var name = options.name ? options.name[level] : '',
		        item = '<select name="'+name+'">' + empty + '</select>';
		    box.append(item);
		}
        ...
        function change(item, level){
            var id = item[ui.value], data = {},
                element = ui.element=='select' ? $(item) : item[ui.element];
            data[_par] = id;
        ...
        return {
            select : function(ids){//重新设置选中项 前提是已初始化一次
