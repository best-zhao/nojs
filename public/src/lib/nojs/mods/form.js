...
define(function( require, $, ui ){
	var form = function(options){
        ...
        this.button = options.button;//指定了button对象，则通过button的click触发submit
        this.formSubmit = this.button && this.button.length ? false : true;//是否为表单提交
        ...
        if( realForm && !this.button ){//默认this.button为真实form中type=submit对象
            this.button = this.form.find('[type="submit"]');
        }
        this.rules = options.rules;
        
        this.item = [];//存储所需验证对象
        options.showIco = options.showIco==false ? false : options.showIco||true;//是否显示提示图标
        options.focusOnError = options.focusOnError==false ? false : true;//错误时是否获得焦点        
        
        this.checkMode = options.checkMode || 'submit';//验证模式 'blur/keyup'
	...
	form.prototype = {
		init : function( rules ){
			...
			var i, j, name,
                _rules = rules || this.rules,
				T = this;
				
			//rules 传入新的规则重新初始化表单				
			this.rules = $.extend( true, {}, _rules );//copy副本
                ...
                (function(m,rule){
                    ...
                    if(m.attr('type')==='password'){//确认密码
                        ...
                        if( c && c['name'] ){
                            ...
                            var _rule = $.extend(true,{}, rule, {isNull : typeof c['isNull']!='undefined' ? c['isNull'] : '请填写确认密码'});
		...
		check : function(m, isSubmit, _rule){
			...
			if( !m.length || ( m.is(':hidden') && type!='hidden' ) ){//隐藏域.is(':hidden')返回true
			    return true;
		    }
		    if( !m.data('formInit') ){//动态添加的元素
		        this.push(name);
		    }
			var T = this,
				n = _rule || m.data('rule'),
