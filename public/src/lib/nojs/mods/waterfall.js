...
define(function(require){
	...
	var waterfall = function(id,opt,load){
		this.element = $("#"+id);			//显示容器
		if(!this.element.length){return;}
		this.opt = opt || {};
		this.item = null;				//列表个体
		this.temp = [];					//存储临时数据，对象
		this.N = null;
		this.state = null;				//取值插入状态
		this.load = waterfall.loadData;	//加载数据函数
		this.ready = null;				//已排序完成的个数
		this.loading = null;			//是否加载数据中
		this.preLoadimg = this.opt.preLoadimg==true ? imgPreLoad() : false;//服务器不返回图片高度时，通过图片预加载获取其高度
		this.onSort = this.opt.onSort;	//排序时回调
		this.page = null;
		if( this.opt.autoWidth || this.opt.autoLoad ){
			this.bindScroll();			//是否自适应宽度/自动加载数据
		}
		this.init();
	}
	waterfall.prototype = {
		init : function(I){
			...
			this.state = true;
			this.page = 1;
			if(!this.item.length){//容器内没有数据	
				//将回调中的this指向当前this;
				//this.opt.autoLoad && this.load.call(this,this.dealData);
			}else{
				for(var i=0;i<this.item.length;i++){
					...
					//h = h.attr('height');
					//m.find(".pic").height(h);//将高度写入到标签中,防止firefox ctrl+f5刷新时读取不到图片高度
				}
		...
		bindScroll : function(){
			...
			function autoLayout(){
				...
				A = window.setTimeout(function(){
					if(T.N.cols && Math.floor((T.element.parent().width()+T.N.margin)/T.N.colWidth)!=T.N.cols){//判断当前列数是否改变
						T.init(true);
					}
				},400);
			}
		...
		getSize : function(obj,col){
			...
			nowCol = parseInt(l/this.N.colWidth);//obj当前所处列			
			
			for(var i=0;i<col;i++){
				this.N.colsPos[nowCol++] = h;//更新当前最低位置
			}
			
			max = Math.max.apply(null,this.N.colsPos);//在所有列中取最大值即为容器高度
			
			max>200 && this.element.height(max);//设置父容器高度
		},
		//重置obj所在列位置
		setCol : function(obj){
			var next = obj.nextAll(),
				...
				col = left/this.N.colWidth,
				h = parseInt(obj.css("top"))+obj.outerHeight(true),
				max = 0;
			for(var i=0;i<len;i++){
				m = next.eq(i);
			...
			this.N.colsPos[col] = parseInt(next.eq(max).css("top"))+next.eq(max).outerHeight(true);//更新最低位置
		}
	}
	...
	waterfall.dataTmpl = function(data){
		var html = [
			'<a class="pic" href="productreview-<%=id%>.html"><img src="<%=pic%>" />',
	...
	waterfall.loadData = function(){
		return function(){
            ...
            var key = this.opt.key,
                url = this.opt.url,
                dataType = this.opt.dataType || 'html';
